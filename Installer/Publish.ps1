# Publish.ps1 — публикует собранный DAN_Plugin (Revit 2023) на сетевую шару обновлений
# K:\02_BIM\DAN_Plugin\2023\, откуда его подхватывают все рабочие станции при следующем
# запуске Revit (см. UpdateSync в Loader.cs). Запускать после Build.ps1 (или сборки
# вручную) — здесь используется уже собранный bin\Release. Параллель Publish2026.ps1.
#
# Использование:
#   .\Publish.ps1 -Version 1.5.0
...
$serverRoot = "K:\02_BIM\DAN_Plugin\2023"

if (-not (Test-Path $binRelease)) {
    Write-Error "Не найдена папка сборки: $binRelease. Сначала выполните Build.ps1."
}
...
Write-Host "`nПубликация DAN_Plugin (Revit 2023) версии $Version на $serverRoot ..." -ForegroundColor Cyan

if (-not (Test-Path $serverRoot)) { New-Item -ItemType Directory -Path $serverRoot -Force | Out-Null }

# Раскладка файлов зеркалит [Files] из DAN_Plugin.iss — то, что окажется в папке
